...
class ObjectIdentityTestCase:
    ...
    def assertDifferentObjects(self, *objs):
        ...
        if count > 1:
            ...
            pytest.fail(f"{dupe} appeared {count} times in {objs}")
    ...
    def test_instance_caching_unary_ops(self):
        f = SomeFactor()
        assert (-f) is (-f)
        assert neg(neg(f)) is neg(neg(f))
        assert neg(neg(neg(f))) is neg(neg(neg(f)))
    ...
    def test_latest_on_different_dtypes(self):
        ...
        for column in TestingDataSet.columns:
            if column.dtype == bool_dtype:
                ...
                pytest.fail(f"Unknown dtype {column.dtype} for column {column}")
    ...
    def test_failure_timing_on_bad_dtypes(self):
        ...
        with pytest.raises(NoDefaultMissingValue) as e:
        ...
        assert str(e.value.args[0]).startswith(
...
class SubDataSetTestCase:
    def test_subdataset(self):
        ...
        for k, some_dataset_column in some_dataset_map.items():
            ...
            assert some_dataset_column is not sub_dataset_column, (
                f"subclass column {k!r} should not have the same identity as the parent"
            )
    ...
    def test_unreasonable_missing_values(self):
        ...
        for base_type, dtype_, bad_mv in (
            ...
            assert re.search(prefix, str(e.value))
