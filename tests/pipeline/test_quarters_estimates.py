...
def make_expected_df_for_factor_compute(start_date, sids, tuples, end_date):
    """
    ...
    df = df.pivot_table(
        columns=SID_FIELD_NAME, values="estimate", index="knowledge_date"
    )
...
class TestNextEstimateMultipleQuarters(WithEstimateMultipleQuarters, ZiplineTestCase):
    ...
    def fill_expected_out(cls, expected):
        ...
        for raw_name in cls.columns.values():
            expected.loc[
                pd.Timestamp("2015-01-01") : pd.Timestamp("2015-01-11"), raw_name + "1"
            ] = cls.events[raw_name].iloc[0]
            expected.loc[
                pd.Timestamp("2015-01-11") : pd.Timestamp("2015-01-20"), raw_name + "1"
        ...
        for col_name in ["estimate", "event_date"]:
            expected.loc[
                pd.Timestamp("2015-01-06") : pd.Timestamp("2015-01-10"), col_name + "2"
