...
class DownsampledPipelineTestCase(WithSeededRandomPipelineEngine, ZiplineTestCase):
    ...
    def check_downsampled_term(self, term):
        ...
        expected_results = {
            "year": (
                raw_term_results.groupby(pd.TimeGrouper("AS"))
                .first()
                .reindex(compute_dates, method="ffill")
            ),
            "quarter": (
                raw_term_results.groupby(pd.TimeGrouper("QS"))
                .first()
                .reindex(compute_dates, method="ffill")
            ),
            "month": (
                raw_term_results.groupby(pd.TimeGrouper("MS"))
                .first()
                .reindex(compute_dates, method="ffill")
            ),
            "week": (
                raw_term_results.groupby(pd.TimeGrouper("W", label="left"))
                .first()
                .reindex(compute_dates, method="ffill")
            ),
        }
