...
EXPECTED_SESSIONS = {
    1: DataFrame(
        ...
        columns=OHLCV,
        index=pd.to_datetime(["2016-03-15"]).as_unit("ns"),
    ),
    2: DataFrame(
    ...
    3: DataFrame(
        ...
        columns=OHLCV,
        index=pd.to_datetime(["2016-03-15", "2016-03-16"]).as_unit("ns"),
    ),
    1001: DataFrame(
        [EXPECTED_AGGREGATION[1001].iloc[-1].values],
        columns=OHLCV,
        index=pd.to_datetime(["2016-03-16"]).as_unit("ns"),
    ),
    1002: DataFrame(
    ...
    1003: DataFrame(
        ...
        columns=OHLCV,
        index=pd.to_datetime(["2016-03-16", "2016-03-17"]).as_unit("ns"),
    ),
    1004: DataFrame(
...
class TestResampleSessionBars(WithFutureMinuteBarReader, ZiplineTestCase):
    ...
    def init_instance_fixtures(self):
        ...
        self.session_bar_reader = MinuteResampleSessionBarReader(
            self.trading_calendar, self.future_minute_bar_reader
        )
    ...
    def test_last_available_dt(self):
        ...
        session_bar_reader = MinuteResampleSessionBarReader(
            calendar, self.future_minute_bar_reader
        )
...
class TestReindexSessionBars(WithEquityDailyBarReader, ZiplineTestCase):
    TRADING_CALENDAR_STRS = ("us_futures", "NYSE")
    TRADING_CALENDAR_PRIMARY_CAL = "us_futures"
    ...
    def test_sessions(self):
        ...
        self.assertEqual(pd.Timestamp("2015-11-02"), sessions[0])
        self.assertEqual(pd.Timestamp("2015-11-30"), sessions[-1])
