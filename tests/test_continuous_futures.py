...
class ContinuousFuturesTestCase(
    ...
    def make_future_minute_bar_data(cls):
        ...
        for i in range(20):
            ...
            if i in sid_to_vol_stop_session:
                ...
                df.iloc[loc, df.columns.get_loc("volume")] = 1000
                df.iloc[loc + 1 :, df.columns.get_loc("volume")] = 0
            j = i - 1
            if j in sid_to_vol_stop_session:
                ...
                if m_close > dts[0]:
                    ...
                    df.iloc[: loc + 1, df.columns.get_loc("volume")] = 10
            if i == 15:  # No volume for MAH16
                df.iloc[:, df.columns.get_loc("volume")] = 0
            if i == 17:
                end_loc = dts.searchsorted("2016-02-16 23:00:00+00:00")
                df.iloc[:end_loc, df.columns.get_loc("volume")] = 10
                df.iloc[end_loc:, df.columns.get_loc("volume")] = 0
            if i == 18:
                ...
                df.iloc[:cross_loc_1, df.columns.get_loc("volume")] = 5
                df.iloc[cross_loc_1:cross_loc_2, df.columns.get_loc("volume")] = 15
                df.iloc[cross_loc_2:cross_loc_3, df.columns.get_loc("volume")] = 5
                df.iloc[cross_loc_3:end_loc, df.columns.get_loc("volume")] = 15
                df.iloc[end_loc:, df.columns.get_loc("volume")] = 0
            if i == 19:
                ...
                df.iloc[:early_cross_1, df.columns.get_loc("volume")] = 1
                df.iloc[early_cross_1:early_cross_2, df.columns.get_loc("volume")] = 20
                df.iloc[early_cross_2:end_loc, df.columns.get_loc("volume")] = 10
                df.iloc[end_loc:, df.columns.get_loc("volume")] = 0
            yield i, df
    ...
    def test_create_continuous_future(self):
        ...
        assert cf_primary.start_date == Timestamp("2015-01-05")
        assert cf_primary.end_date == Timestamp("2022-09-19")
    ...
    def test_current_contract(self):
        ...
        assert contract.symbol == "FOG16", (
            "Auto close at beginning of session so FOG16 is now the current contract."
        )
...
""")
        ...
        assert result.primary.symbol == "FOF16", (
            "Primary should be FOF16 on first session."
        )
        assert result.secondary.symbol == "FOG16", (
...
""")
    ...
    def test_history_sid_minute(self):
        ...
        assert window.loc["2016-01-27 22:32", cf.sid] == 1, (
            "Should be FOG16 at beginning of window."
        )

        assert window.loc["2016-01-27 23:01", cf.sid] == 1, (
    ...
    def test_history_close_session(self):
        ...
        window = self.data_portal.get_history_window(
            [cf.sid], Timestamp("2016-03-06"), 30, "1d", "close", "daily"
        )
        ...
        # Advance the window a month.
        window = self.data_portal.get_history_window(
            [cf.sid], Timestamp("2016-04-06"), 30, "1d", "close", "daily"
        )
    ...
    def test_history_close_session_adjusted(self):
        ...
        window = self.data_portal.get_history_window(
            [cf, cf_mul, cf_add], Timestamp("2016-03-06"), 30, "1d", "close", "daily"
        )
        ...
        assert_almost_equal(
            ...
            124992.348,
            err_msg="At beginning of window, should be FOG16's first value, adjusted.",
        )
        ...
        assert_almost_equal(
            ...
            125241.440,
            err_msg="On session with roll, should be FOH16's 24th value, unadjusted.",
        )
        ...
        # Advance the window a month.
        window = self.data_portal.get_history_window(
            [cf, cf_mul, cf_add], Timestamp("2016-04-06"), 30, "1d", "close", "daily"
        )
        ...
        assert_almost_equal(
            window.loc["2016-02-29", cf_add],
            135281.44,
            err_msg="On session after roll, should be FOH16's 25th value, unadjusted.",
        )
        ...
        assert_almost_equal(
            ...
            135431.44,
            err_msg="On session with roll, should be FOJ16's 43rd value, unadjusted.",
        )
    ...
    def test_history_close_minute(self):
        ...
        assert window.loc["2016-02-25 22:32", cf.sid] == 115231.412, (
            "Should be FOG16 at beginning of window. A minute "
            "which is in the 02-25 session, before the roll."
        )

        assert window.loc["2016-02-25 23:00", cf.sid] == 115231.440, (
            "Should be FOG16 on on minute before roll minute."
        )

        assert window.loc["2016-02-25 23:01", cf.sid] == 125240.001, (
        ...
        assert window.loc["2016-02-26 22:32", cf.sid] == 125241.412, (
            "Should be FOH16 at beginning of window."
        )

        assert window.loc["2016-02-28 23:01", cf.sid] == 125250.001, (
...
class OrderedContractsTestCase(zf.WithAssetFinder, zf.ZiplineTestCase):
    ...
    def test_contract_at_offset(self):
        ...
        assert 1 == oc.contract_at_offset(1, 0, start_dates[-1].value), (
            "Offset of 0 should return provided sid"
        )

        assert 2 == oc.contract_at_offset(1, 1, start_dates[-1].value), (
    ...
    def test_delivery_predicate(self):
        ...
        oc = OrderedContracts(
            "BA", contracts, chain_predicate=partial(delivery_predicate, {"F", "H"})
        )
