...
class ExamplesTests(WithTmpDir, ZiplineTestCase):
    ...
    def init_class_fixtures(cls):
        ...
        cls.expected_perf = dataframe_cache(
            cls._expected_perf_dir(cls.tmpdir.getpath(
                'example_data/expected_perf',
            )),
    ...
    @staticmethod
    def _expected_perf_dir(root):
        """The expected results for the running pandas version, or else the
        newest checked-in results (generated by the original zipline).
        """
        current = os.path.join(root, pd.__version__.replace('.', '-'))
    ...
    def test_example(self, example_name, benchmark_returns):
        ...
        assert_equal(
            ...
            check_dtype=False,
            # Expected results pickled by old pandas have object columns.
