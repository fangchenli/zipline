...
"""
        self.run_algorithm(script=algo_text)
...
def log_nyse_close(context, data):
    ...
    def test_schedule_function(self):
        ...
        def incrementer(algo, data):
            ...
            assert curdt == datetime.datetime.combine(
                curdt.date(),
                datetime.time(9, 31),
                tzinfo=us_eastern,
            )
...
class TestBeforeTradingStart(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_data_in_bts_minute(self):
        ...
        assert 352 == algo.history_values[0]["high"][2].iloc[19]
    ...
    def test_data_in_bts_daily(self):
        ...
        assert 350, results.the_price2.iloc[0]

        assert 392 == algo.history_values[0]["high"][1].iloc[0]
        assert 390 == algo.history_values[0]["price"][1].iloc[0]

        assert 352 == algo.history_values[0]["high"][2].iloc[0]
        assert 350 == algo.history_values[0]["price"][2].iloc[0]
    ...
    def test_account_bts(self):
        ...
        assert results.port_value.iloc[1] == pytest.approx(
            10000 + 780 - 392 - 0, abs=1e-2
        )
    ...
    """,
            ...
            if minimum_commission == 0:
                ...
                for order_ in all_orders:
                    assert order_["filled"] * 0.02 == pytest.approx(
                        order_["commission"], abs=1e-7
                    )
            else:
                ...
                for order_ in all_orders:
                    if order_["filled"] > 0:
                        assert max(
                            order_["filled"] * 0.02, minimum_commission
                        ) == pytest.approx(order_["commission"], abs=1e-7)
                    else:
...
def handle_data(context, data):
    ...
    def test_arg_types(self, name, inputs):
        ...
        expected = "Expected {} argument to be of type {}{}".format(
            keyword, "or iterable of type " if inputs[2] else "", inputs[1]
        )
...
"""
        ...
        if change_loc == "interday":
            assert algo.capital_change_deltas == {pd.Timestamp("2006-01-04"): 1000.0}
        else:
    ...
    def test_capital_changes_minute_mode_minute_emission(self, change, values):
        ...
        capital_changes = {
            _capital_change_key(val[0], change_loc): {
                "type": change_type,
                "value": val[1],
            }
            for val in values
...
class TestDailyEquityAutoClose(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_daily_delisted_equities(self, order_size, capital_base):
        ...
        last_minute_of_session = self.trading_calendar.session_last_minute(
            self.test_days[1]
        )
    ...
    def test_cancel_open_orders(self):
        ...
        last_close_for_asset = algo.trading_calendar.session_last_minute(
...
class TestMinutelyEquityAutoClose(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def final_minute_price(self, asset):
        return (
            ...
            .loc[self.trading_calendar.session_last_minute(asset.end_date)]
            .close
        )
    ...
    def test_order_in_quiet_period(self, caplog, name, sid):
        ...
        with caplog.at_level(logging.WARNING, logger="zipline"):
            ...
            for w in warnings:
                ...
                assert expected_message == w.getMessage()
