# Generated by scripts/gen_api_stub.py; do not edit by hand.
...
from zipline.algorithm import OnError, TradingAlgorithm
from zipline.assets import Asset, ContinuousFuture, Equity, Future
from zipline.finance import cancel_policy as cancel_policy
from zipline.finance import commission as commission
from zipline.finance import execution as execution
from zipline.finance import slippage as slippage
from zipline.finance.asset_restrictions import RESTRICTION_STATES as RESTRICTION_STATES
from zipline.finance.asset_restrictions import (
    HistoricalRestrictions as HistoricalRestrictions,
)
from zipline.finance.asset_restrictions import Restriction as Restriction
from zipline.finance.asset_restrictions import Restrictions
from zipline.finance.asset_restrictions import StaticRestrictions as StaticRestrictions
from zipline.finance.cancel_policy import CancelPolicy
from zipline.finance.cancel_policy import EODCancel as EODCancel
from zipline.finance.cancel_policy import NeverCancel as NeverCancel
...
from zipline.finance.slippage import FixedSlippage as FixedSlippage
from zipline.finance.slippage import SlippageModel
from zipline.finance.slippage import VolumeShareSlippage as VolumeShareSlippage
from zipline.pipeline import Pipeline
from zipline.protocol import Order
from zipline.sources.requests_csv import PandasRequestsCSV
from zipline.utils import events as events
from zipline.utils import math_utils as math_utils
from zipline.utils.events import EventRule
from zipline.utils.events import calendars as calendars
from zipline.utils.events import date_rules as date_rules
from zipline.utils.events import time_rules as time_rules
...
def batch_market_order(
    share_counts: pd.Series[int] | pd.Series[float],
) -> list[str | None]:
...
def cancel_order(order_param: str | Order) -> None:
...
def fetch_csv(
    ...
    special_params_checker: Callable[[str], tuple[str, dict[str, str] | None]]
    | None = None,
...
def get_datetime(tz: tzinfo | str | None = None) -> pd.Timestamp:
...
@overload
def get_environment(field: Literal["platform", "arena"] = "platform") -> str:
...
@overload
def get_environment(field: Literal["data_frequency"]) -> Literal["daily", "minute"]: ...
@overload
def get_environment(field: Literal["start", "end"]) -> pd.Timestamp: ...
@overload
def get_environment(field: Literal["capital_base"]) -> float: ...
@overload
def get_environment(field: Literal["*"]) -> dict[str, str | pd.Timestamp | float]: ...
def get_open_orders(
...
def history(
    bar_count: int, frequency: str, field: str, ffill: bool = True
) -> pd.DataFrame:
...
def set_asset_restrictions(
    restrictions: Restrictions, on_error: OnError = "fail"
) -> None:
...
def set_do_not_order_list(
    restricted_list: Iterable[Asset] | SecurityList, on_error: OnError = "fail"
) -> None:
    """Set a restriction on which assets can be ordered.
...
def set_long_only(on_error: OnError = "fail") -> None:
...
def set_max_order_count(max_count: int, on_error: OnError = "fail") -> None:
...
def set_min_leverage(min_leverage: float, grace_period: pd.Timedelta) -> None:
...
def set_slippage(
    us_equities: SlippageModel | None = None, us_futures: SlippageModel | None = None
) -> None:
...
def set_symbol_lookup_date(dt: datetime | str) -> None:
...
def symbol(symbol_str: str, country_code: str | None = None) -> Equity:
...
def symbols(*args: str, country_code: str | None = None) -> list[Equity]:
