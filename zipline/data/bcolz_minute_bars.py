...
class BcolzMinuteBarWriter:
    ...
    def __init__(
        ...
        self._market_opens = calendar.first_minutes.loc[start_session:end_session]
    ...
    def pad(self, sid, date):
        ...
        if pd.isnull(last_date):
            ...
            days_to_zerofill = tds[
                tds.slice_indexer(start=last_date + self._calendar.day, end=date)
            ]
...
class H5MinuteBarUpdateWriter:
    ...
    # Version 1 stores a (sid, dt)-indexed frame; version 0 stored a pd.Panel.
    FORMAT_VERSION = 1
    ...
    def write(self, frames):
        """
        Write the frames to the target HDF5 file as a single table indexed by
        ``(sid, dt)``.
        ...
        with HDFStore(
            ...
            store.put("updates", updates)
        with tables.open_file(self._path, mode="r+") as h5file:
            h5file.set_node_attr("/", "version", self.FORMAT_VERSION)
...
class H5MinuteBarUpdateReader(MinuteBarUpdateReader):
    ...
    def read(self, dts, sids):
        ...
        for sid in sids:
            yield sid, updates.xs(sid, level="sid").reindex(dts)


def minute_value(market_opens, pos, minutes_per_day):
    """
    ...
    """
    day, minute = divmod(pos, minutes_per_day)
    return market_opens[day] + minute


def find_position_of_minute(
    market_opens, market_closes, minute_val, minutes_per_day, forward_fill
):
    """
    ...
    """
    market_open_loc = np.searchsorted(market_opens, minute_val, side="right") - 1
...
def find_last_traded_position_internal(
    market_opens, market_closes, end_minute, start_minute, volumes, minutes_per_day
):
    """
    ...
    minute_pos = min(
        find_position_of_minute(
            market_opens, market_closes, end_minute, minutes_per_day, True
        ),
        len(volumes) - 1,
    )
    while minute_pos >= 0:
        current_minute = minute_value(market_opens, minute_pos, minutes_per_day)
        day = minute_pos // minutes_per_day
        if current_minute > market_closes[day]:
            minute_pos = find_position_of_minute(
                market_opens,
                market_closes,
                market_closes[day],
                minutes_per_day,
